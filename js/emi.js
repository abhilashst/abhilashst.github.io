...
function setUpHandlers() {
    ...
    var laSlider = new Slider("#la", { formatter: sliderFormatter });
    var nmSlider = new Slider("#nm", { formatter: sliderFormatter });
    var roiSlider = new Slider("#roi", { formatter: sliderFormatter });

    laSlider.on("slide", function(data) {
    $("#la_value").html(parseInt(data).toFixed(0));
    calculateEMI();
  });

    nmSlider.on("slide", function(event, data) {
    $("#nm_value").html(parseInt(data).toFixed(0));
    calculateEMI();
  });

    roiSlider.on("slide", function(event, data) {
    $("#roi_value").html(parseInt(data).toFixed(0));
    calculateEMI();
  });
